...
export const AdminProductsPage = () => {
  ...
  const fields = [
    ...
    { name: 'category', label: 'Category' }, { name: 'material', label: 'Material' },
    { name: 'roomCategory', label: 'Room Category' }, { name: 'shortDescription', label: 'Short Description', full: true },
    { name: 'description', label: 'Description', type: 'textarea', full: true }, { name: 'featured', label: 'Featured', type: 'checkbox' },
  ];
  const submit = async (e) => {
    e.preventDefault();
    const payload = { ...form, price: Number(form.price || 0), images: [{ url: form.images?.[0] || baseProduct.images[0], alt: form.name }] };
    editingId ? await updateProduct(editingId, payload) : await createProduct(payload);
    setForm(baseProduct); setEditingId(''); reload();
  };
  return <AdminShell title="Products" description="Create, edit, and remove catalogue products."><div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]"><div className="overflow-x-auto rounded-[1.5rem] border border-black/5 p-5"><table className="min-w-full text-sm"><thead><tr className="text-stone"><th className="py-3 text-left">Product</th><th>Category</th><th>Price</th><th>Status</th><th></th></tr></thead><tbody>{items.map((item)=><tr key={item._id} className="border-t border-silk"><td className="py-3"><div className="font-medium text-charcoal">{item.name}</div><div className="text-stone">{item.shortDescription}</div></td><td>{titleCase(item.category)}</td><td>{formatCurrency(item.price)}</td><td>{titleCase(item.status)}</td><td className="space-x-2"><button className="text-stone" onClick={()=>{setEditingId(item._id);setForm({...item,images:[item.coverImage||item.images?.[0]?.url||baseProduct.images[0]]});}}>Edit</button><button className="text-red-600" onClick={async()=>{await deleteProduct(item._id);reload();}}>Delete</button></td></tr>)}</tbody></table>{error ? <div className="mt-4 text-red-600">{error}</div> : null}</div><div className="rounded-[1.5rem] border border-black/5 p-5"><h2 className="font-serif-display text-3xl text-charcoal">{editingId ? 'Edit Product' : 'Create Product'}</h2><div className="mt-4"><PanelForm fields={fields} form={form} setForm={setForm} onSubmit={submit} submitLabel="product" editing={!!editingId} /></div></div></div></AdminShell>;
};

export const AdminProjectsPage = () => {
  const { items, error, reload } = useCrud(() => getProjects({ limit: 100 }));
  const [form, setForm] = useState(baseProject); const [editingId, setEditingId] = useState('');
  const fields = [{name:'title',label:'Title'},{name:'location',label:'Location'},{name:'year',label:'Year',type:'number'},{name:'category',label:'Category'},{name:'summary',label:'Summary',full:true},{name:'description',label:'Description',type:'textarea',full:true},{name:'featured',label:'Featured',type:'checkbox'}];
  const submit = async (e)=>{e.preventDefault(); const payload={...form, year:Number(form.year||new Date().getFullYear()), images:[{url:form.images?.[0]||baseProject.images[0],alt:form.title}]}; editingId? await updateProject(editingId,payload): await createProject(payload); setForm(baseProject); setEditingId(''); reload();};
  return <AdminShell title="Projects" description="Manage project portfolio entries."><div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]"><div className="overflow-x-auto rounded-[1.5rem] border border-black/5 p-5"><table className="min-w-full text-sm"><thead><tr className="text-stone"><th className="py-3 text-left">Project</th><th>Location</th><th>Year</th><th></th></tr></thead><tbody>{items.map((item)=><tr key={item._id} className="border-t border-silk"><td className="py-3"><div className="font-medium text-charcoal">{item.title}</div><div className="text-stone">{item.summary}</div></td><td>{item.location}</td><td>{item.year}</td><td className="space-x-2"><button className="text-stone" onClick={()=>{setEditingId(item._id);setForm({...item,images:[item.coverImage||item.images?.[0]?.url||baseProject.images[0]]});}}>Edit</button><button className="text-red-600" onClick={async()=>{await deleteProject(item._id);reload();}}>Delete</button></td></tr>)}</tbody></table>{error ? <div className="mt-4 text-red-600">{error}</div> : null}</div><div className="rounded-[1.5rem] border border-black/5 p-5"><h2 className="font-serif-display text-3xl text-charcoal">{editingId ? 'Edit Project' : 'Create Project'}</h2><div className="mt-4"><PanelForm fields={fields} form={form} setForm={setForm} onSubmit={submit} submitLabel="project" editing={!!editingId} /></div></div></div></AdminShell>;
};
...
export const AdminUsersPage = () => {
  ...
  return <AdminShell title="Users" description="Activate or deactivate customer accounts."><div className="overflow-x-auto rounded-[1.5rem] border border-black/5 p-5"><table className="min-w-full text-sm"><thead><tr className="text-stone"><th className="py-3 text-left">User</th><th>Status</th><th>Joined</th><th>Action</th></tr></thead><tbody>{items.map((item)=><tr key={item._id} className="border-t border-silk"><td className="py-3"><div className="font-medium text-charcoal">{item.name}</div><div className="text-stone">{item.email}</div></td><td>{item.isActive ? 'Active' : 'Inactive'}</td><td>{shortDate(item.createdAt)}</td><td><button className="rounded-full border border-charcoal px-4 py-2 text-xs uppercase tracking-[0.24em]" onClick={async()=>{await updateAdminUser(item._id,{isActive:!item.isActive});reload();}}>{item.isActive ? 'Deactivate' : 'Activate'}</button></td></tr>)}</tbody></table>{error ? <div className="mt-4 text-red-600">{error}</div> : null}</div></AdminShell>;
};
